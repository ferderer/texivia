...
/** Represents the router configuration object. */
interface RouterOptions<T extends BaseRouteConfig, C = any> {
...
interface BaseRouteConfig {
  ...
  hooks?: Hook<BaseRouteConfig, any>[];   // Optional hooks for route lifecycle
...
/** The Router class for handling navigation and route matching. */
class Router<T extends BaseRouteConfig, C = null> {
  ...
  private routerName: string | null = null;

  /**
   * Initialize the router with routes configuration and options.
   *
   * @param routes - A map of routes, including a 'notFound' route.
   * @param options - Configuration options including global hooks, prefix, and event name.
   */
  constructor(routes: Record<string, T> = {}, options: RouterOptions<T, C> = {}) {
    ...
    this.routes = {};
    this._compile(routes);
  }

  /**
   * Compiles the user-provided routes into a format usable by the router.
  ...
  private _process(routeConfig: T, name: string): CompiledRoute<T> {
    ...
    const SEGMENT_REGEX = /(?:\/([^\/{}]+)|\/\{([a-zA-Z_$][a-zA-Z0-9_$]*)(?::([^/]+))?\}|(\/))/g;
   ...
   */
  private _matchRoute(path: string): { route: CompiledRoute<T>; params: Record<string, string> } | null {
    if (!this.mappings)
      throw new Error('Router not compiled');

    const matches = this.mappings.exec(path);
    if (!matches || matches[0] !== path || !matches.groups)
      return this.ignoreUnknown || !this.routes.notFound ? null : { route: this.routes.notFound, params: {} };

    const name = Object.keys(matches.groups ?? {}).find((key) => matches.groups?.[key] !== undefined);
    if (!name) {
  ...
  /** Starts the router, attaching event listeners and navigating to the current path. */
  start(): this {
    window.addEventListener('popstate', this._handlePopState.bind(this));
    this.interceptLinks && document.addEventListener('click', this._handleClick.bind(this));
    this.delegateUnknown && document.addEventListener(`${this.eventName}-delegate`, this._onDelegate.bind(this));
    this.navigate(window.location.pathname + window.location.search, false);
    return this;
  }

  /** Stops the router, removing event listeners. */
  stop(): void {
    window.removeEventListener('popstate', this._handlePopState);
    this.interceptLinks && document.removeEventListener('click', this._handleClick);
  }
   ...
   */
  generateUrl(name: string, params: Record<string, string> = {}, query: Record<string, string> = {}): string {
    const route = this.routes[name];
    if (!route) {
    ...
    for (const paramName of route.params) {
      ...
      const paramRegex = new RegExp(`\\{${escapedParamName}(?::[^}]*)?\\}`, 'g');
  ...
  /**
   * Handles custom event triggered by another instance of the router.
   *
   * @param event - The custom event containing the path another router had no config for.
   * @returns A promise that resolves when the delegate action is complete.
   */
  private async _onDelegate(e: Event): Promise<void> {
    const event = e as CustomEvent;
    if (event.detail.router !== this.routerName) {
   ...
   * @param delegate - Whether to delegate to another router if no match is found.
   * @returns A promise resolving to the matched route and params, or false if navigation is canceled.
   */
  async navigate(path: string, pushState: boolean = true, redirectCount: number = 0):
   Promise<{ route: CompiledRoute<T>; params: Record<string, string> } | false> {
    ...
    if (match === null) {
      this.delegateUnknown &&
        document.dispatchEvent(new CustomEvent(`${this.eventName}-delegate`, { detail: { path, router: this.routerName } }));
      return false;
    }
   ...
   * @returns A promise resolving to the matched route and params, or false if navigation is canceled.
   */
  async navigateToNamed(name: string, params: Record<string, string> = {}, pushState: boolean = true):
    Promise<{ route: CompiledRoute<T>; params: Record<string, string> } | false> {
   ...
   */
  private _dispatchEvent(route: CompiledRoute<T>, params: Record<string, string>): void {
  ...
  private async _handlePopState(event: PopStateEvent): Promise<void> {
    await this.navigate(window.location.pathname + window.location.search, false);
  }
